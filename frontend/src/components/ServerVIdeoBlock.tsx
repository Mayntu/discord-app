...
const ServerVideoBlock:FC=()=> {
    const [clients,setClients] = useStateWithCallback([])
    const {idVoiceChst} = useParams()
    ...
    const addNewClients = useCallback((newClient:string,cb:()=>void)=>{
      ...
      useEffect(()=>{
        ...
        if(idVoiceChst){
          ...
          .then(()=>{
            let user = $api.get("api/v1/getUsersInfo")
            user.then((res)=>{
               socketWebRTC.emit("join-room",{room_id :idVoiceChst, name:  res.data.user_data.login})
            })
...
export default   ServerVideoBlock